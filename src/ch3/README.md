...
在这一章, chapter 3, 我们会自己动手实现一个线程池! 听起来是不是超级厉害(≧∇≦)ﾉ

会有一点难度, 但是耐心可能的解决的, 方法总比困难多!

首先我们需要知道什么是"锁"

还记得ch1中我们说的"临界资源"嘛? 简单回顾下, "临界资源" 同一时刻只能被一个线程能拥有. 硬件和操作系统不像人一样. 如果是人, 当多个人都想使用卫生间的时候, 能快速讨论出一个使用顺序, 但是对于计算机, 我们必须在资源上显式的加一个标志, 告诉计算机现在谁能拥有这个被多个线程共享的"临界资源".

所以我们需要"锁", 用一把"锁"把共享资源锁住, 同时只给多个线程一把钥匙. 谁抢到谁就能"解锁".
锁是在通过硬件提供的指令和操作系统在软件层面共同实现的. 可以说"锁"是一种特殊类型的变量, 虽然"锁"不是我们接触过的INT/DOUBLE之类的类型. (感兴趣的同学可以看看 CAS(compare and swap)这个硬件实现的功能, 以及如何通过CAS实现spin_lock. 还有操作系统管理的waitqueue实现进程的挂起休眠, 每个"资源"都有一个waitqueue, 线程排队等待, 不用忙等待了, 可以降低性能损耗.)

不要多想, 锁真的只有我们上面说的那么简单. 背后没有那么多玄奥秘密, 锁就是为了锁住资源. 当然锁的实现也有不同方式, 锁的使用场景也是千奇百怪. 不过这些都不是现在我们需要考虑的.

来看看在多线程编程中, 怎么使用锁吧.

C/CPP提供了pthread, 也就是POSIX Thread.
...
```
我们在一个名为ThreadPool的结构体中定义了一把名为mutex的互斥锁. 放在ThreadPool中是因为我们希望pthread_mutex_t mutex能保护住ThreadPool.
