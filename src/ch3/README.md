...
锁是通过硬件提供的指令和操作系统在软件层面共同实现的. 可以说"锁"是一种特殊类型的变量, 虽然"锁"不是我们接触过的INT/DOUBLE之类的类型. (感兴趣的同学可以看看 CAS(compare and swap)这个硬件实现的功能, 以及如何通过CAS实现spin_lock. 还有操作系统管理的waitqueue实现进程的挂起休眠, 每个"资源"都有一个waitqueue, 线程排队等待, 不用忙等待了, 可以降低性能损耗.)
